...
class KittiDataset(torch.utils.data.Dataset):
    def __init__(self, root, mode, valid):
        ...
        deeplab101.load_state_dict(torch.load('D:/AI/KITTI/training/deeplab_20epochs.pth', map_location=torch.device('cpu')))
            ...
            with open(label_path) as f:
                ...
                for l in lines:
                    if l.startswith('Car'):
                        ...
                        # height(x-axis of camera [right/left]), width(y axis of camera [down/up]),
                        # length(z axis of camera[forward/back]) of car (meters)
    ...
    def create_class_scores_mask(self, img):
        ...
        mask = mask['out']  # ignore auxiliary output
        _, preds = torch.max(mask, 1)
        class_scores = torch.where(preds == 3, torch.ones(preds.shape).to(self.device), torch.zeros(preds.shape).
                                   to(self.device))  # convert preds to binary map (1 = car, else 0)
    ...
    def augment_lidar_class_scores(self, class_scores, lidar_cam_coords, projection_mats):
        ...
        # indexing order below is 1 then 0 because points_projected_on_mask is x,y in image coords which is cols, rows while class_score shape is (rows, cols)
        point_scores = class_scores[points_projected_on_mask[:, 1].type(torch.LongTensor),
                                    points_projected_on_mask[:, 0].type(torch.LongTensor)].reshape(-1, 1).double()
